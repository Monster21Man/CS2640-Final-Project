# CS2640.02 Group 5 Final Project
# Logan Bailey, Marco Joson, Russell Salvador, Jay Wu
# December 7th, 2025
# Final Project: Tic Tac Toe
# A complete Tic Tac Toe game in assembly that uses a bitmap to display information
# This file should be located in the same directory as the file: "Bitmap.asm"


.macro getInt(%reg)          # get int input from user
li $v0, 5
syscall
move %reg, $v0
.end_macro

.macro exit                  # exit program
li $v0, 10
syscall
.end_macro

.macro printDataString(%str) # print a .asciiz label
li $v0, 4
la $a0, %str
syscall
.end_macro

.macro intCheckErrorXchord
check:	#reads 1 char as string from input to check for char(invalid input)
li $v0, 8
la $a0, buffer
li $a1, 2
syscall

lb $t9, 0($a0)

blt $t9, '0', Invalid ##checks user input is 0 through 9 and not a char, if char reprompt
bgt $t9, '9', Invalid
j end

Invalid: 
lw $t0, currentPlayer #checks current player to proceed with right string
li $t1, 1
beq $t0, $t1, promptPlayer1
printDataString(charError)
printDataString(user2XPrompt)

j check

promptPlayer1:
printDataString(charError)
printDataString(user1XPrompt)

j check

end:
subi $t9, $t9, '0' # subtracts ascii value of given number to 0 to get int value
move $t7, $t9	#stores int value into $t7 to check for valid chords
.end_macro

.macro intCheckErrorYchord
check:	#reads 1 char as string from input to check for char(invalid input)
li $v0, 8
la $a0, buffer
li $a1, 2
syscall

lb $t9, 0($a0)

blt $t9, '0', Invalid #checks user input is 0 through 9 and not a char, if char then reprompt
bgt $t9, '9', Invalid
j end

Invalid: 
lw $t0, currentPlayer #checks current player to proceed with right string
li $t1, 1
beq $t0, $t1, promptPlayer1
printDataString(charError)
printDataString(user2YPrompt)

j check

promptPlayer1:
printDataString(charError)
printDataString(user1YPrompt)

j check

end:
subi $t9, $t9, '0' # subtracts ascii value of given number to 0 to get int value
move $t6, $t9 #stores int value into $t6 to check for valid chords
.end_macro

# Data

.data
# Bitmap display settings (needed for Bitmap.asm functions)
# IMPORTANT: Set Bitmap Display tool to base address 0x10008000 (heap)
displayAddress: .word 0x10008000
white:     .word 0xFFFFFF
black:     .word 0x000000
gridColor: .word 0x808080    # Gray
xColor:    .word 0xFF0000    # Red for X
oColor:    .word 0x0000FF    # Blue for O

# Y/N for menu
charY: .asciiz "Y"
charN: .asciiz "N"

# Menu text
menu1: .asciiz "Tic Tac Toe\n"
menu2: .asciiz "-------------\n"
menu3: .asciiz "\nTo start press Y\nTo exit press N\n"
invalidYN: .asciiz "\nNot Valid Input, please Enter Y or N\n"

startMsg: .asciiz "\n\nGame Started\n---------------\n"
exitMsg: .asciiz "\n\nExiting... Thanks for Playing!\n"

# Turn
breakLine: .asciiz "\n|---------------------------------------------------------|\n"
user1Turn: .asciiz "\nIt is Player One's turn!"
user1XPrompt: .asciiz "\nPlace your 'X' x-cord (Choose 1-3): "
user1YPrompt: .asciiz "\nPlace your 'X' y-cord (Choose 1-3): "
user2Turn: .asciiz "\nIt is Player Two's turn!"
user2XPrompt: .asciiz "\nPlace your 'O' x-cord (Choose 1-3): "
user2YPrompt: .asciiz "\nPlace your 'O' y-cord (Choose 1-3): "
invalidInput: .asciiz "\nError! Invalid coordinate input. Please choose a number 1, 2, or 3."
errTaken: .asciiz "\nThat spot is already taken. Choose another.\n"

#win messages
p1WinMsg: .asciiz "\nPLAYER 1 (X) WINS!\n"
p2WinMsg: .asciiz "\nPLAYER 2 (O) WINS!\n"
catsGameMsg: .asciiz "\nCAT'S GAME – NO WINNER!\n"
#turn counter
moveCount: .word 0

#error checks
charError: .asciiz "\nInvalid Input, Integer only\n"


buffer: .space 2

# Game state
# board: 9 bytes, 0 = empty, 1 = X, 2 = O
board: .space 9
currentPlayer: .word 1      # 1 = Player 1 (X), 2 = Player 2 (O)


# Text / Code
.text
.globl main

# main: print menu, handle Y/N, start or exit

main:
# Initialize the bitmap display (from Bitmap_Display.asm)
jal clearScreen
jal drawGrid

# Print menu
printDataString(menu1)
printDataString(menu2)
printDataString(menu3)

MenuInput:
li $v0, 12        # read character
syscall
move $s0, $v0       # s0 = input char

lb $t0, charY     # first byte of "Y"
beq $s0, $t0, startGame

lb $t1, charN     # first byte of "N"
beq $s0, $t1, exitProgram

# Invalid Y/N
printDataString(invalidYN)
j MenuInput



# startGame: clear board, reset player, loop

startGame:
# Clear board (set all 9 cells to 0)
la $t0, board
li $t1, 0          # index 0..8
li $t2, 0
sw $t2, moveCount


clearLoop:
beq $t1, 9, boardCleared
sb $zero, 0($t0)   # store 0 (empty)
addi $t0, $t0, 1
addi $t1, $t1, 1
j clearLoop

boardCleared:
# Clear the display and redraw grid (from Bitmap_Display.asm)
jal clearScreen
jal drawGrid

printDataString(startMsg)

li $t0, 1
sw $t0, currentPlayer



gameLoop:
jal getMove
jal checkWin

lw $t0, currentPlayer
li $t1, 1
beq $t0, $t1, switchTo2

li $t0, 1
sw $t0, currentPlayer
j gameLoop

switchTo2:
li $t0, 2
sw $t0, currentPlayer
j gameLoop

# exitProgram: print goodbye and exit
exitProgram:
printDataString(exitMsg)
exit
    
getMove:
lw $t0, currentPlayer

li $t1, 1
beq $t0, $t1, getMoveP1   # if player 1
j getMoveP2             # else player 2

# getMoveP1: Player One (X)

getMoveP1:
printDataString(breakLine)
printDataString(user1Turn)

GetP1X:
printDataString(user1XPrompt)
intCheckErrorXchord()
             # t7 = x (1..3) -> col

# validate 1 <= x <= 3
li $t1, 1
blt $t7, $t1, errorCoordP1
li $t1, 3
bgt $t7, $t1, errorCoordP1

GetP1Y:
printDataString(user1YPrompt)
intCheckErrorYchord()
            # t6 = y (1..3) -> row

# validate 1 <= y <= 3
li $t1, 1
blt $t6, $t1, errorCoordP1
li $t1, 3
bgt $t6, $t1, errorCoordP1

j checkCell

errorCoordP1:
printDataString(invalidInput)
j getMoveP1

# getMoveP2: Player Two (O)

getMoveP2:
printDataString(breakLine)
printDataString(user2Turn)

GetP2X:
printDataString(user2XPrompt)
intCheckErrorXchord()
             # t7 = x (1..3) -> col

# validate 1 <= x <= 3
li $t1, 1
blt $t7, $t1, errorCoordP2
li $t1, 3
bgt $t7, $t1, errorCoordP2

GetP2Y:
printDataString(user2YPrompt)
intCheckErrorYchord()
              # t6 = y (1..3) -> row

# validate 1 <= y <= 3
li $t1, 1
blt $t6, $t1, errorCoordP2
li $t1, 3
bgt $t6, $t1, errorCoordP2

j checkCell

errorCoordP2:
printDataString(invalidInput)
j getMoveP2



checkCell:
# Convert (row, col) from 1-3 to index 0-8:
addi $t1, $t6, -1         # t1 = row-1
addi $t2, $t7, -1         # t2 = col-1

li $t3, 3
mul $t3, $t1, $t3       
add $t3, $t3, $t2        

la $t4, board
add $t4, $t4, $t3        

# Check if cell is empty (0)
lb $t5, 0($t4)
bne $t5, $zero, cellTaken

# Cell is empty -> store currentPlayer (1 or 2) as byte
sb $t0, 0($t4)

# increment move counter
lw  $t9, moveCount
addi $t9, $t9, 1
sw  $t9, moveCount

# Draw the move on the bitmap (from Bitmap_Display.asm)
# Save registers we need to preserve
addi $sp, $sp, -12
sw $ra, 0($sp)
sw $t6, 4($sp)   # row
sw $t7, 8($sp)   # col

# Set up arguments for drawing
move $a0, $t7    # col (1-3)
move $a1, $t6    # row (1-3)
move $a2, $t0    # player (1 or 2)
jal drawMove

# Restore registers
lw $ra, 0($sp)
lw $t6, 4($sp)
lw $t7, 8($sp)
addi $sp, $sp, 12

jr $ra                  # done, return to gameLoop


cellTaken:
printDataString(errTaken)

# Reprompt same player based on currentPlayer
li $t1, 1
beq $t0, $t1, getMoveP1  # if player 1
j getMoveP2            # else player 2




#checks if a player has won the game
#this checks for 8 possible win conditions
#reference board:
#	[0, 1, 2]
#	[3, 4, 5]
#	[6, 7, 8]
checkWin:

la $t0, board      #pointer to board base

#check rows

li $t1, 0          #row index (0 to 2)

row_loop:
li $t2, 3
mul $t3, $t1, $t2     #t3 = row*3 (check every 3rd index)

add $t8, $t0, $t3
lb  $t4, 0($t8)
beq $t4, $zero, next_row

lb  $t5, 1($t8)
bne $t4, $t5, next_row

lb  $t6, 2($t8)
bne $t4, $t6, next_row

j winner_found

next_row:
addi $t1, $t1, 1
li $t7, 3
blt $t1, $t7, row_loop


#check columns

li $t1, 0          #col index (0 to 2)

col_loop:
add $t8, $t0, $t1
lb  $t4, 0($t8)
beq $t4, $zero, next_col

lb  $t5, 3($t8)
bne $t4, $t5, next_col

lb  $t6, 6($t8)
bne $t4, $t6, next_col

j winner_found

next_col:
addi $t1, $t1, 1
li $t7, 3
blt $t1, $t7, col_loop


#check diagonal 0, 4, 8

lb $t4, 0($t0)
beq $t4, $zero, diag2

lb $t5, 4($t0)
bne $t4, $t5, diag2

lb $t6, 8($t0)
bne $t4, $t6, diag2

j winner_found


#check other diagonal #2, 4, 6
diag2:
lb $t4, 2($t0)
beq $t4, $zero, no_winner

lb $t5, 4($t0)
bne $t4, $t5, no_winner

lb $t6, 6($t0)
bne $t4, $t6, no_winner

j winner_found


#no winner condition
no_winner:
lw  $t9, moveCount
li  $t1, 9
bne $t9, $t1, continue_game

# 9 moves and no winner → tie
printDataString(breakLine)
printDataString(catsGameMsg)
exit

continue_game:
jr $ra


#player win condition
winner_found:
li $t1, 1
beq $t4, $t1, player1_wins #player 1 wins

# else player 2
printDataString(breakLine)
printDataString(p2WinMsg) #player 2 win message
exit

#player 1 win message
player1_wins:
printDataString(breakLine)
printDataString(p1WinMsg)
exit


# Include bitmap display functions from separate file
.include "Bitmap.asm"